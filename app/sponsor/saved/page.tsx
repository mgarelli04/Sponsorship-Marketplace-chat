...
import { marketplaceCategories, marketplaceCreators } from "@/src/data/sponsor-marketplace";
...
export default function SponsorSaved() {
  ...
  const savedCreators = useMemo(() => {
    ...
    const visible = marketplaceCreators.filter((creator) => {
  ...
  const totalBudget = savedCreators.reduce((sum, creator) => sum + creator.packages[0].price, 0);
                  ...
                  <div className="mt-4">
                    ...
                    <div className="flex items-center justify-between gap-2">
                      <p className="text-sm font-bold text-[#6c7c96]">
                        From {formatCurrency(creator.packages[0].price)}
                      </p>
                      <Link
