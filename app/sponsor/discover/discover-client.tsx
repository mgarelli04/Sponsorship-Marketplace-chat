...
export default function SponsorDiscoverClient({
  creators,
  categories,
  sourceStatus,
  sourceMessage,
  ...
  categories: string[];
  sourceStatus: "connected" | "missing-env" | "query-error";
    ...
    const results = creators.filter((creator) => {
      ...
      const matchesBudget =
        ...
        (packagePrice !== undefined &&
          ((budgetRange === "Under $5K" && packagePrice < 5000) ||
            (budgetRange === "$5K - $15K" && packagePrice >= 5000 && packagePrice <= 15000) ||
            ...
            {filteredCreators.length === 0 ? (
              <div className="mt-4 rounded-lg border border-dashed border-[#d9e2ef] bg-white p-8 text-center text-sm text-[#64748b]">
                {sourceStatus === "connected" ? (
                  ...
                  <span>{sourceMessage ?? "Discover data is not available from the database."}</span>
                )}
              </div>
            ) : null}
