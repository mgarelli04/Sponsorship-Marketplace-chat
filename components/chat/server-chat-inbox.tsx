...
export default async function ServerChatInbox({
  role,
  selectedThreadId,
}: {
  role: ChatRole;
  ...
  const script = selectedThread?.status === "accepted"
    ? `
(function () {
  ...
  function appendMessage(message) {
    ...
    var mine = message.senderUserId === ${JSON.stringify(user.id)};
      ...
      <div className="mx-auto max-w-6xl px-6 py-10">
        <div className="mb-6">
          ...
          <p className="mt-2 text-sm text-[#64748b]">
            Chat con aceptacion, persistencia en base de datos y WebSocket cuando la conversacion esta aceptada.
          </p>
        </div>

        <div className="grid gap-6 lg:grid-cols-[320px_1fr]">
          <aside className="rounded-2xl border border-[#d9e2ef] bg-white p-4 shadow-sm">
            <h2 className="mb-3 text-sm font-bold text-[#0f172a]">Conversaciones</h2>
                ...
                {threads.map((thread) => (
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      <p className="font-semibold text-[#0f172a]">{thread.counterpartName}</p>
